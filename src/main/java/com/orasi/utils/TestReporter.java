...
public class TestReporter {
	...
	/**
	 * This method will assert for condition true and log in the report. it will be a soft assert that means
	 * testmethod execution will not stop.
	 * @param condition - Boolean condition to be tested
	 * @param description - Assertion description you want to login
	 * 
	 * @author SDUBEY
	 * @since 11 April 2016
	 */
	public static SoftAssert  softAssertTrue(SoftAssert softAssert,boolean condition, String description) {	
		try{
//			SoftAssert softAssert=new SoftAssert();
			softAssert.assertTrue(condition, description);
			
		}catch (AssertionError failure){
			logFailure("Soft Assert True - " + description );
			if(getPrintToConsole()) System.out.println(getTimestamp() + "Assert True - " + trimHtml(description));
			Assert.fail(description);
		}
//		Reporter.log(getTimestamp() + " <font size = 2 color=\"green\"><b><u>Assert True - " + description + "</font></u></b><br />");
//		if(getPrintToConsole()) System.out.println(getTimestamp() + "Assert True - " + trimHtml(description));
	...
	public static void assertAll(SoftAssert sAssert){
		try{
			sAssert.assertAll();
		}catch (AssertionError failure){
			String errorMessge=failure.toString();
			errorMessge=errorMessge.replaceAll("failed:", "failed:<br>");
			errorMessge=errorMessge.replaceAll(", ", "<br>");
			logFailure("Soft Assert Failed - " + errorMessge );
			errorMessge=errorMessge.replaceAll("<br>", "\n");
			
			if(getPrintToConsole()) System.out.println(getTimestamp() + "Assert True - " + trimHtml(errorMessge));
			Assert.fail(errorMessge);
		}
	}
	
	/**
	 * This method will report Failure in selenium report reporter output as well as mark the fail in TestNG also
	 * @param failMessage - Message you want to log in reports and testNG
	 * 
	 * @author SDUBEY
	 * @since 11 April 2017
	 */
	public static void ReportFail(String failMessage){
		TestReporter.logFailure(failMessage);
